...
import fetchOtherUsersPosts from '../../actions/workingWithPost/getOtherUsersPosts';
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            try {
                ...
                const otherUsersPosts = await fetchOtherUsersPosts(token);
                setPosts(otherUsersPosts);
            } catch (err) {
                console.error('Failed to fetch other users posts:', err);
                setError(err instanceof Error ? err.message : 'Не удалось загрузить посты других пользователей');
            } finally {
                setIsLoading(false);
            }
        };
        ...
        return (
            <div className={cn(backgroundClasses, 'flex items-center justify-center')}>
                <div className="text-center">
                    ...
                    <p className={cn('mt-4', mutedTextClasses)}>
                        Загрузка постов...
                    </p>
                </div>
            </div>
            ...
            <div className={cn(backgroundClasses, 'flex items-center justify-center')}>
                <div className="text-center">
                    ...
                    <h2 className={cn('text-2xl font-semibold mb-2', textClasses)}>
                        Пока нет постов от других пользователей
                    </h2>
                    <p className={mutedTextClasses}>
                        Другие пользователи еще не опубликовали посты
                    </p>
                </div>
            </div>
        );
    ...
    return (
        <div className={cn(backgroundClasses, 'py-8')}>
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="mb-8">
                    <h1 className={cn('text-3xl font-bold', textClasses)}>
